...
import {classificationSummary,pendingMedicines,parseBackup,presentInOrder,type Evaluation} from '@/lib/queue';
...
export default function Home(){
 ...
 async function analyze(){
  ...
  try{
   for(let offset=0;offset<queue.length;offset+=12){
    ...
    setPresenting(false);setActiveId(chunk[0].id);setSelectedId(chunk[0].id);
    ...
    if(!mounted.current)break;
    // The API measures the Gateway round trip; presentation time is excluded.
    setElapsed(total=>total+data.elapsedMs/1000);
    ...
    setPresenting(true);
    await presentInOrder(chunk,(m,i)=>{
     setActiveId(m.id);setSelectedId(m.id);
     setBatch({done:offset+i+1,total:queue.length});
    },controller.signal);
   }
  }finally{lock.current=false;if(mounted.current){setRunning(false);setPresenting(false);setActiveId(null)}}
 }
 ...
 <div className="dashboard-stats"><div><span>LİSTEDEKİ İLAÇ</span><strong>{medicines.length.toString().padStart(2,'0')}</strong></div><div><span>DEĞERLENDİRİLEN</span><strong>{completed.length.toString().padStart(2,'0')}</strong></div><div><span>BEKLEYEN</span><strong>{pending.length.toString().padStart(2,'0')}</strong></div><div><span>TOPLAM TOKEN</span><strong>{totalTokens.toLocaleString('tr-TR')}</strong></div><div><span title="Son çalışmadaki başarılı Jev isteklerinin toplam yanıt süresi; ekranda gösterim süresi dahil değildir.">JEV SÜRESİ</span><strong>{running&&!presenting?'…':elapsed.toFixed(1)}<small> sn</small></strong></div></div>
 ...
 <section className="panel queue-panel"><div className="panel-head"><div className="panel-title"><Pill/><h2>Eklenen ilaçlar</h2></div><span className="count">{medicines.length} KAYIT</span></div><div className="queue-list">{!medicines.length?<div className="compact-empty"><Pill/><p>Eklediğin ilaçlar burada birikecek.</p></div>:medicines.map((m,i)=><div className={'queue-item'+(activeId===m.id?' current':'')} key={m.id}><span className="number">{String(i+1).padStart(2,'0')}</span><div className="queue-item-name"><strong>{m.name}</strong><span>{m.notes||'Ek kutu bilgisi yok'}{m.expiry?` · SKT ${m.expiry.slice(5)}/${m.expiry.slice(2,4)}`:''}</span></div><span className={'queue-state '+(evaluations[m.id]?'done':failures[m.id]?'failed':'')}>{evaluations[m.id]?<><Check/>Tamamlandı</>:activeId===m.id?<><LoaderCircle className="spin"/>İşleniyor</>:failures[m.id]?<><RotateCcw/>Tekrar dene</>:<><Clock/>Bekliyor</>}</span>{!evaluations[m.id]&&<button className="icon-button" aria-label={`${m.name} kaydını kaldır`} disabled={running} onClick={()=>remove(m.id)}><X/></button>}</div>)}</div>
   <div className="queue-controls"><button className="primary analysis-button" onClick={analyze} disabled={running||!pending.length}>{running?<LoaderCircle className="spin"/>:<Activity/>}{running?`${presenting?'Sonuçlar gösteriliyor':'Değerlendiriliyor'} · ${batch.done}/${batch.total}`:pending.length?`Jev ile değerlendir (${pending.length})`:'Jev ile değerlendir'}{!running&&<ArrowRight/>}</button><div className="backup-row"><button type="button" onClick={exportJSON} disabled={running||!medicines.length}><Download/>JSON indir</button><button type="button" onClick={()=>fileInput.current?.click()} disabled={running}><Upload/>JSON yükle</button><input ref={fileInput} type="file" accept="application/json,.json" hidden onChange={e=>{const f=e.target.files?.[0];if(f)void importFile(f);e.target.value=''}}/></div><p className="hint">{running?'Yeni eklediklerin bir sonraki çalışmada işlenir.':'Bekleyen kayıtlar en fazla 12’şer toplu gönderilir; yanıtlar canlı panelde liste sırasıyla gösterilir.'}</p></div></section>
